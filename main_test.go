...
func TestApis(t *testing.T) {
	...
	cases := []Case{
		...
		// alexpa: чтобы тест стал проходимым, в CREATE TABLE было добавлено
		// значение по умолчанию для поля info, а здесь добавено поле email.
		// в противном случае, т.к. оба этих поля not null и has no default,
		// правильный ответ должен быть: 400 - field %s is required.
		Case{
			Path:   "/users/",
			Method: http.MethodPut,
			Body: CR{
		...
		// alexpa: спорное решение, imho: 400 - limit must be int >= 1
		Case{
			Path:  "/users",
			Query: "limit=1'&offset=1\"",
			Result: CR{
